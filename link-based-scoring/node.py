...
'''
	Node class used for computing link based scoring of weapon ads json files
'''
class Node(object):
	nodeId = ''
	location =''
	...
	def __init__(self, nodeId, location, country, title, description, sellerDesc, sellerStartDate, postUrl, 
		...
		'''
			constructor, all nodes are initialized with a score of 0.5 for start of scoring computations
		'''
		self.nodeId = nodeId
...
def getNodeFromJson(aJson):
	'''
	Extracts field values from aJson and return a node object with assigned object variables
	'''
